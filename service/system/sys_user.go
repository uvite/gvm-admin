...
func (userService *UserService) DeleteUser(id request.IdsReq) (err error) {
	var user system.SysUser
	err = global.GVA_DB.Where("id in (?)", id.Ids).Delete(&user).Error
	if err != nil {
		return err
	}
	err = global.GVA_DB.Delete(&[]system.SysUserAuthority{}, "sys_user_id in (?)", id.Ids).Error
	return err
}
