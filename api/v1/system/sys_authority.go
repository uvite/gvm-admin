...
func (a *AuthorityApi) DeleteAuthoritys(c *gin.Context) {
	...
	if err := authorityService.DeleteSystemRoleByIds(IDS); err != nil {
...
func (a *AuthorityApi) GetAuthorityTree(c *gin.Context) {

	list, err := authorityService.GetAuthorityInfoTree()
	if err != nil {
		global.GVA_LOG.Error("获取失败!", zap.Error(err))
		response.FailWithMessage("获取失败"+err.Error(), c)
		return
